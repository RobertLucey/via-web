...
export function roadRows(features = [], merge = false) {
  ...
  return [...groups.values()].map(({ name, features }, index) => {
    const values = (field) => features.map(f => measurement(f.properties[field], field === "avg")).filter(v => v !== null);
    ...
    return { id: index, name, quality: average(values("avg")), usage: usage.length ? Math.max(...usage) : null, speed: speed === null ? null : speed * 3.6, features };
  });
}
...
export function visibleFeatures(features, bounds) {
  ...
  return features.filter(feature => {
    const points = feature.geometry.coordinates.flat(feature.geometry.type === "MultiLineString" ? 1 : 0);
    const lngs = points.map(p => p[0]);
    const lats = points.map(p => p[1]);
    return Math.max(...lngs) >= bounds.getWest() && Math.min(...lngs) <= bounds.getEast() && Math.max(...lats) >= bounds.getSouth() && Math.min(...lats) <= bounds.getNorth();
  });
}

export function displayValue(value) {
  return value === null || value === undefined ? "No data" : Number(value.toFixed(1)).toLocaleString();
}
